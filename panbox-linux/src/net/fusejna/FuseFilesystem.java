...
public abstract class FuseFilesystem {
	...
	private static final String defaultFilesystemName = "panboxfs-";
	private static final Pattern regexNormalizeFilesystemName = Pattern
			.compile("[^a-zA-Z]");
	...
	@FuseMethod
	final int _fgetattr(final String path, final StructStat stat,
			final StructFuseFileInfo info) {
	...
	@FuseMethod
	final int _fsync(final String path, final int datasync,
			final StructFuseFileInfo info) {
		return fsync(path, datasync, new FileInfoWrapper(path, info));
	}

	@FuseMethod
	final int _fsyncdir(final String path, final int datasync,
			final StructFuseFileInfo info) {
		final FileInfoWrapper wrapper = new FileInfoWrapper(path, info);
	...
	@FuseMethod
	final int _ftruncate(final String path, final TypeOff offset,
			final StructFuseFileInfo info) {
		final FileInfoWrapper wrapper = new FileInfoWrapper(path, info);
	...
	@FuseMethod
	final int _getxattr(final String path, final String xattr,
			final Pointer buffer, final TypeSize size, final TypeUInt32 position) {
		final long sizeValue = size.longValue();
		if (buffer == null || sizeValue == 0L) {
			return 0;
		}
		final ByteBuffer buf = buffer.getByteBuffer(0, sizeValue);
		final int result = getxattr(path, xattr, buf, sizeValue,
				position == null ? 0L : position.longValue());
		if (result == 0) {
			try {
				buf.put((byte) 0);
	...
	@FuseMethod
	final int _listxattr(final String path, final Pointer buffer,
			final TypeSize size) {
		final long sizeValue = size.longValue();
		if (buffer == null || sizeValue == 0L) {
			return 0;
		...
		final XattrListFiller filler = new XattrListFiller(
				buffer.getByteBuffer(0, sizeValue), sizeValue);
		return listxattr(path, filler);
	}

	@FuseMethod
	final int _lock(final String path, final StructFuseFileInfo info,
			final int cmd, final StructFlock flock) {
		...
		final int result = lock(path, fileWrapper, FlockCommand.fromBits(cmd),
				flockWrapper);
		fileWrapper.write();
		flockWrapper.write();
		return result;
	...
	@FuseMethod
	final int _read(final String path, final Pointer buffer,
			final TypeSize size, final TypeOff offset,
			final StructFuseFileInfo info) {
	...
	@FuseMethod
	final int _readlink(final String path, final Pointer buffer,
			final TypeSize size) {
		final long bufSize = size.longValue();
		final ByteBuffer buf = buffer.getByteBuffer(0, bufSize);
	...
	@FuseMethod
	final int _setxattr(final String path, final Pointer buffer,
			final TypeSize size, final int flags, final TypeUInt32 position) {
		final long sizeValue = size.longValue();
		final ByteBuffer buf = buffer.getByteBuffer(0, sizeValue);
		return setxattr(path, buf, sizeValue, flags, position == null ? 0L
				: position.longValue());
	}
	...
	@FuseMethod
	final int _write(final String path, final Pointer buffer,
			final TypeSize size, final TypeOff offset,
			final StructFuseFileInfo info) {
		final long bufSize = size.longValue();
	...
	@UserMethod
	public abstract int create(final String path, final ModeWrapper mode,
			final FileInfoWrapper info);

	/**
	 * Subclasses may override this to customize the default parameters applied
	 * to the stat structure, or to prevent such behavior (by overriding this
	 * method with an empty one)
	 * 
	 * @param wrapper
	 *            The
	 */
	protected void defaultStat(final StatWrapper wrapper, final long uid,
			final long gid, final long lastModified) {
		// Set some sensible defaults
		wrapper.setMode(NodeType.DIRECTORY).setAllTimesMillis(lastModified)
				.nlink(1).uid(FuseJna.getUid()).gid(FuseJna.getGid());
	}

	protected void defaultStat(final StatWrapper wrapper, final long uid,
			final long gid) {
		// Set some sensible defaults
		// wrapper.setMode(NodeType.DIRECTORY).nlink(1).uid(FuseJna.getUid())
		// .gid(FuseJna.getGid());
		wrapper.nlink(1).uid(FuseJna.getUid()).gid(FuseJna.getGid());
	}
	...
	@UserMethod
	public abstract int fgetattr(final String path, final StatWrapper stat,
			final FileInfoWrapper info);
	...
	@UserMethod
	public abstract int fsync(final String path, int datasync,
			final FileInfoWrapper info);

	@UserMethod
	public abstract int fsyncdir(final String path, int datasync,
			final FileInfoWrapper info);

	@UserMethod
	public abstract int ftruncate(final String path, final long offset,
			final FileInfoWrapper info);
	...
	/**
	 * Returns the raw fuse_context structure. Only valid when called while a
	 * filesystem operation is taking place.
	 * 
	...
	protected final StructFuseContext getFuseContext() {
		if (!isMounted()) {
			throw new IllegalStateException(
					"Cannot get FUSE context if the filesystem is not mounted.");
		}
		return FuseJna.getFuseContext();
	}

	/**
	 * Returns the gid field of the fuse context. Only valid when called while a
	 * filesystem operation is taking place.
	 * 
	 * @return The group ID of the process executing an operation on this
	 *         filesystem.
	 */
	...
	/**
	 * Returns the pid field of the fuse context. Only valid when called while a
	 * filesystem operation is taking place.
	 * 
	 * @return The process ID of the process executing an operation on this
	 *         filesystem.
	 */
	...
	/**
	 * Returns the uid field of the fuse context. Only valid when called while a
	 * filesystem operation is taking place.
	 * 
	 * @return The user ID of the user running the process executing an
	 *         operation of this filesystem.
	 */
	...
	@UserMethod
	public abstract int getxattr(final String path, final String xattr,
			final ByteBuffer buf, final long size, final long position);
	...
	public abstract int listxattr(final String path,
			final XattrListFiller filler);

	@UserMethod
	public abstract int lock(final String path, final FileInfoWrapper info,
	...
	protected final FuseFilesystem log(final boolean logging) {
		return log(logging ? Logger.getLogger(getClass().getCanonicalName())
				: null);
	}
	...
	protected final FuseFilesystem log(final Logger logger) {
		...
		if (mountPoint != null) {
			mountLock.unlock();
			throw new IllegalStateException(
					"Cannot turn logging on/orr when filesystem is already mounted.");
		}
		FuseFilesystem.logger = logger;
		mountLock.unlock();
		return this;
	}
	...
	@UserMethod
	public abstract int mknod(final String path, final ModeWrapper mode,
			final long dev);

	public final void mount(final File mountPoint, final PanboxFS panboxFS)
			throws FuseException {
		mount(mountPoint, true, panboxFS);
	}

	public void mount(final File mountPoint, final boolean blocking,
			final PanboxFS panboxFS) throws UnsatisfiedLinkError,
			FuseException {
		mountLock.lock();
		if (isMounted()) {
			throw new IllegalStateException(getFuseName()
	...
	public final void mount(final String mountPoint, final PanboxFS panboxFS)
			throws FuseException {
		mount(new File(mountPoint), true, panboxFS);
	}
	...
	@UserMethod
	public abstract int read(final String path, final ByteBuffer buffer,
	...
	@UserMethod
	public abstract int readlink(final String path, final ByteBuffer buffer,
			final long size);
	...
	@UserMethod
	public abstract int setxattr(final String path, final ByteBuffer buf,
			final long size, final int flags, final long position);
	...
	public abstract int utimens(final String path,
			final TimeBufferWrapper wrapper);

	@UserMethod
	public abstract int write(final String path, final ByteBuffer buf,
